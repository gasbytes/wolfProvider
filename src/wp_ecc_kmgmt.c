 ...
 *
 * The imported point is always checked against the curve as
 * wc_ecc_import_x963_ex() only does so when wolfSSL is built with
    ...
    word32 len)
{
    ...
    if (ok) {
        rc = wc_ecc_copy_point((ecc_point*)&pub.pubkey, &ecc->key.pubkey);
        if (rc != 0) {
